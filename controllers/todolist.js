...
export const updateTodo = async (req, res) => {
    try {
        ...
        // Only allow updating if the todo belongs to the authenticated user
...
export const deleteTodo = async (req, res) => {
    try {
        const { id } = req.params;
        // Only allow deleting if the todo belongs to the authenticated user
        const deletedTodo = await TodolistModel.findOneAndDelete({ _id: id, user: req.user.id });
